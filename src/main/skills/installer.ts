 ...
 *
 * Respects user-managed skills: if a skill dir exists without .orbiter-version,
...
const VERSION_FILE = '.orbiter-version'
...
function log(msg: string): void {
  ...
  try { appendFileSync(join(homedir(), '.orbiter-debug.log'), line) } catch {}
}
...
async function installGithubSkill(
  ...
  try {
    ...
    // Atomic swap: remove old (if Orbiter-managed), rename tmp into place
    if (existsSync(targetDir)) {
      const existing = readVersionFile(targetDir)
      if (existing?.installedBy === 'orbiter') {
...
async function installSkill(
  ...
  if (existsSync(targetDir)) {
    ...
    if (!meta) {
      // Dir exists but no .orbiter-version — user-managed, don't touch
      log(`Skipping ${entry.name}: user-managed (no ${VERSION_FILE})`)
      onStatus({ name: entry.name, state: 'skipped', reason: 'user-managed' })
      return
    }

    if (meta.version === entry.version && meta.installedBy === 'orbiter') {
