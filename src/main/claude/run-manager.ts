...
const DEBUG = process.env.ORBITER_DEBUG === '1'

// Appended to Claude's default system prompt so it knows it's running inside Orbiter.
// Uses --append-system-prompt (additive) not --system-prompt (replacement).
const ORBITER_SYSTEM_HINT = [
  'IMPORTANT: You are NOT running in a terminal. You are running inside Orbiter,',
  'a desktop chat application with a rich UI that renders full markdown.',
  'Orbiter is a GUI wrapper around Claude Code — the user sees your output in a',
  'styled conversation view, not a raw terminal.',
  '',
  'Because Orbiter renders markdown natively, you MUST use rich formatting when it helps:',
  '- Always use clickable markdown links: [label](https://url) — they render as real buttons.',
  '- When the user asks for images, and public web images are appropriate, proactively find and render them in Orbiter.',
  ...
  '- Do not ask whether Orbiter can render images; assume it can.',
...
// safety. This is a deliberate Orbiter policy choice, not native Claude parity.
...
export class RunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    if (options.hookSettingsPath) {
      // Orbiter-scoped hook settings: the PreToolUse HTTP hook handles permissions
    ...
    // Always tell Claude it's inside Orbiter (additive, doesn't replace base prompt)
    args.push('--append-system-prompt', ORBITER_SYSTEM_HINT)
