 ...
 *
 * This is a Orbiter-owned permission broker that approximates Claude Code's
 ...
 *   - Per-run settings files (only Orbiter-spawned sessions see the hook)
 */
...
export class PermissionServer extends EventEmitter {
  ...
  generateSettingsFile(runToken: string): string {
    ...
    const dir = join(tmpdir(), 'orbiter-hook-config')
    try { mkdirSync(dir, { recursive: true, mode: 0o700 }) } catch {}

    const filePath = join(dir, `orbiter-hook-${runToken}.json`)
