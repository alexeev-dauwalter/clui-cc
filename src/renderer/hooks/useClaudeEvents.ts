...
export function useClaudeEvents() {
  ...
  useEffect(() => {
    ...
    const unsubEvent = window.orbiter.onEvent((tabId, event) => {
    ...
    const unsubStatus = window.orbiter.onTabStatusChange((tabId, newStatus, oldStatus) => {
      handleStatusChange(tabId, newStatus, oldStatus)
    })

    const unsubError = window.orbiter.onError((tabId, error) => {
      handleError(tabId, error)
    })

    const unsubSkill = window.orbiter.onSkillStatus((status) => {
      if (status.state === 'failed') {
        console.warn(`[Orbiter] Skill install failed: ${status.name} — ${status.error}`)
      }
    })
  ...
  // Note: window.orbiter.start() is called via sessionStore.initStaticInfo() in App.tsx.
