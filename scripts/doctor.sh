...
if [ "$(uname)" = "Darwin" ]; then
  ...
  check "OS" "0" "unsupported OS ($(uname))"
fi
...
# Build toolchain (platform-specific)
if [ "$(uname)" = "Darwin" ]; then
  # Xcode CLT
  ...
  if command -v gcc &>/dev/null; then
    gver=$(gcc --version 2>&1 | head -1)
    check "gcc" "1" "$gver"
  else
    check "gcc" "0" "not found — sudo dnf install gcc-c++ (Fedora) or sudo apt install build-essential (Debian)"
  fi
  if command -v g++ &>/dev/null; then
    check "g++" "1" "$(g++ --version 2>&1 | head -1)"
  else
    check "g++" "0" "not found — sudo dnf install gcc-c++ (Fedora) or sudo apt install build-essential (Debian)"
  fi
  if command -v make &>/dev/null; then
    check "make" "1" "$(make --version 2>&1 | head -1)"
  else
    check "make" "0" "not found — sudo dnf install make (Fedora) or sudo apt install make (Debian)"
  fi
fi
