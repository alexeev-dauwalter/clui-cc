...
Orbiter is a **macOS-only Electron overlay** that wraps the Claude Code CLI (`claude -p --output-format stream-json`) in a floating pill UI. It is NOT a web app, NOT a VS Code extension, and does NOT call the Anthropic API directly — it spawns CLI subprocesses.
...
| Debug logging | `ORBITER_DEBUG=1 npm run dev` (writes to `~/.orbiter-debug.log`) |
...
| **Preload** | `src/preload/` | Typed IPC bridge (`window.orbiter` API). Security boundary. |
...
```
InputBar.tsx → window.orbiter.prompt(tabId, requestId, opts)
...
4. Call from renderer via `window.orbiter.*`
