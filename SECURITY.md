...
If you discover a security vulnerability in Orbiter, please report it responsibly:
...
Orbiter runs entirely on your local machine. Key security properties:
...
- **Preload isolation** — the renderer has no direct access to Node.js APIs; all IPC goes through a typed `window.orbiter` bridge.
...
No other network connections are made by Orbiter itself. The `claude` CLI may make its own connections as part of normal operation.
